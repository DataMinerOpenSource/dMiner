...
public class LinearOrdinaryLeastSquaresTest {
    ...
    @Test
    public void testRegressStrongLinearFunction() {
        // y = 2x + 4
        ...
        Assert.assertEquals(2.0, results.getCoefficientA(), Double.MIN_NORMAL);
        Assert.assertEquals(4.0, results.getCoefficientB(), Double.MIN_NORMAL);
        Assert.assertTrue(results.getCorrelationCoefficient() > 0.7);
        Assert.assertEquals(0.0, results.getRootMeanSquaredError(), Double.MIN_NORMAL);
        Assert.assertArrayEquals(inputData[1], predictedFunctionValues, Double.MIN_NORMAL);
    }

    @Test
    public void testNonCorrelatedData() {
        final double[][] nonCorrelatedData = {
            {1, 2, 3, 4, 5, 6, 7},
            {1, 1, 3, 8, 2, 1, 0},
        };

        final OrdinaryLeastSquares osl = new LinearOrdinaryLeastSquares();
        final RegressionResults results = osl.regress(nonCorrelatedData);

        // correlation = -0.1147
        Assert.assertTrue(Math.abs(results.getCorrelationCoefficient()) < 0.7);
