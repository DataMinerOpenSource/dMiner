...
public class HyperbolicOrdinaryLeastSquaresTest {
    @Test
    public void testRegressStrongHyperbolicFunction() {
        // y = 1 / 2x + 2
        ...
        Assert.assertEquals(0.5, results.getCoefficientA(), 0.0000001);
        Assert.assertEquals(2.0, results.getCoefficientB(), 0.0000001);
        Assert.assertTrue(Math.abs(results.getCorrelationCoefficient()) > 0.7);
        Assert.assertEquals(1.897084619569385E-10, results.getRootMeanSquaredError(), Double.MIN_NORMAL);
        Assert.assertArrayEquals(inputData[1], predictedFunctionValues, 0.000000001);
    }
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testInputValueIsEqualToZero() {
        final double[][] inputData = {
            {0, 1, 2, 3, 4, 5, 6},
